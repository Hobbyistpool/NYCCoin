...
#if defined(NDEBUG)
# error "newyorkc cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5597f25c062a3038c7fd815fe46c67dedfcb3c839fbc8e01ed4044540d08fe48");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // newyorkc: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "newyorkc Signed Message:\n";
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // newyorkc: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04AFC6B9D279C647C67C250BC4C68F5E9BD714867D8D45B6E54FEEFFA6CB9074EE7CBF69EB8FF388304604C2E00661601502B42E3CF71E934B4A5EAB0D5B209BFD") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // newyorkc: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if(pfrom->strSubVer != "/Satoshi:1.0.0.1/" && pfrom->strSubVer != "/Satoshi:1.0.1.1/" && strstr(pfrom->strSubVer.c_str(), "NewYorkCoin") == NULL)
        {
            printf("%s using version %i %s from other client; disconnecting\n", pfrom->addr.ToString().c_str(), pfrom->nVersion, pfrom->strSubVer.c_str());
            pfrom->fDisconnect = true;
            return false;
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("newyorkcMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("newyorkcMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static newyorkcMiner(CWallet *pwallet)
{
    printf("newyorkcMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("newyorkc-miner");
    ...
    try { loop {
        ...
        printf("Running newyorkcMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("newyorkcMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&newyorkcMiner, pwallet));
}
